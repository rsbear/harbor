...
<script lang="ts">
    ...
    import CommandHistory from "./history/CommandHistory.svelte";

    import { mode } from "@harbor/api/mode";
    ...
    onMount(async () => {
        ...
        console.log("pre setTimouet mode.current()", mode.current());
        setTimeout(() => {
            mode.setMode("PICK");
            console.log(
                "post inside setTimouet mode.current()",
                mode.current(),
            );
        }, 200);
        console.log("post setTimouet mode.current()", mode.current());
    });

    function onChange(event: Event) {
        const eventInput = event.target as HTMLInputElement;
        input.value.set(eventInput.value);
    }

    async function handleSubmit(event: Event) {
        event.preventDefault();
        const miniName = input.miniName.get();
        cmd = miniName;
